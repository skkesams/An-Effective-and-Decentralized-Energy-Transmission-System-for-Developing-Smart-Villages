function router_linking()      
    ...
    router_object.graph_coordinates = rand(n,3);                       
    % this sets value of x and y co-ordinates 
                                                                           
    router_object.distance_matrix    = [];                             
    % initialize  the distance and cost matrix to null
    
    router_object.Initial_Populaton_size    = input('Please enter initial population size : ');    
    % this is the initial population size for GA          
         
        
    router_object.no_of_generations  = input('Please enter no. of iterations for GA to run : ');  
         
    router_object.show_progress = true;                                      
    % show_progress will show the GA progress if true     
    % % ---------- true and True have different meaning as matlab is case sensitive language --------- 
    
    router_object.display_results = true;                                    
    % display_results  shows the GA results if  set to true
    ...
    xyz_coordiantes_of_one_element     =   router_object.graph_coordinates;                  
    % graph_coordinates (float) is an Nx2 matrix of city locations, where N is the number of routers
    
    distance_matrix  =   router_object.distance_matrix;              
    %  distance_matrix (float) is an NxN matrix of point to point distances/costs
    
    Initial_Populaton_size   = router_object.Initial_Populaton_size;         
    %  Initial_Populaton_size (scalar integer) is the size of the population (should be divisible by 4)
    
    no_of_generations  = router_object.no_of_generations;      
    %  no_of_generations (scalar integer) is the number of desired iteration_noations for the algorithm to run
    
    show_progress         = router_object.show_progress;                     
    %  show_progress (scalar logical) shows the GA progress if true
    
    display_results       = router_object.display_results;                   
    %  display_results (scalar logical) shows the GA results if true
  ...
  %  a'=  1     1     1
   %      1     2     2
    %     3     3     3
        ...
        distance_matrix = reshape(sqrt(sum((xyz_coordiantes_of_one_element(a,:)-xyz_coordiantes_of_one_element(a',:)).^2,2)),no_of_routers,no_of_routers);   
        % reshape(A,n1,n2) returns the n1 * n2 matrix, which has the same elements as A. The elements are taken column-wise from A to fill in the elements of the n1-by-n2 matrix.
    end
    ...
    % makes initial population size as multiple of 2
    
    init_population = zeros(Initial_Populaton_size,n);                        
    % generate an empty initial pop , where no of columns will decide no of router/elements
    
       
    for k = 1:Initial_Populaton_size;                                        
        % for all other population values the  initial population is generted randomly
         
        init_population(k,1) = 1; 
        init_population(k,n) = n;
        
       init_population(k,2:n-1) = 1+randperm(n-2)                                    
    ...
    temprory_four_individuals = zeros(4,n);                                        
    % this will only store values of 4 generations
    new_population = zeros(Initial_Populaton_size,n);                       
    %  this will store values of all generations
    
    
    if show_progress                        
        %  % we can choose not to show graph by setting value of this varible to false
         figure('Name','Simple Genetic Algoithm','Numbertitle','off');      
         % it will show the graph of progress
  ...
  %% calculate total distance from a router
    ...
    for iteration_no = 1:no_of_generations
        ...
        for p = 1:Initial_Populaton_size                                    
            % this algorithm will calculate distance for every individual and then  select
            d = 0;                                                           
            % for every iteration_noation the value of distance is set to 0 in- order to recalsulate it
